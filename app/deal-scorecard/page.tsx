...
import { calculateScores, type FormData, type ScoreResults } from "./lib/calculations"
...
export default function DealScorecard() {
  ...
  const [datocmsId, setDatocmsId] = useState<string | null>(null) // NEW: Store DatoCMS record ID
  ...
  // NEW: Second API call when financing data changes
  useEffect(() => {
  ...
  const handleEmailCapture = async (email: string) => {
    try {
      ...
      // Send to DatoCMS and Zapier webhook (EXISTING CALL - NO CHANGES)
      const leadData = {
        ...formData,
        email,
        scores,
      ...
      // DatoCMS integration (EXISTING - NO CHANGES)
      ...
      // NEW: Store the DatoCMS record ID for future updates
